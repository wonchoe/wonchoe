...
def build():
    ...
    strip = []
    for row, sites in enumerate(SITES):
        y = 304 + row * 24
        sx = SEL["x"]
        for i, site in enumerate(sites):
            if i:
                strip.append(f'<path class="sep" d="M{sx:.0f} {y-8}v9"/>')
                sx += 15
            strip.append(f'{T.run("mono", site, 11.5, sx, y, 0.2)}')
            sx += T.width("mono", site, 11.5, 0.2) + 15
  ...
  <style>
    ...
    .creds  {{ fill:#93A6D6 }}
    .strip  {{ fill:#7184B8 }}
    .sep    {{ stroke:#33427A; stroke-width:1 }}
  ...
  <path class="rule" d="M52 280H690" opacity=".7"/>
  <g class="strip">{"".join(strip)}</g>
